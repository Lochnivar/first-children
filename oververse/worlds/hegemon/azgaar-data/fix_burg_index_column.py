#!/usr/bin/env python3
"""
Script to fix Index column in burgs.md - restore numeric indices
"""
...
# Culture index to name mapping (reverse for checking)
culture_name_to_index = {
    "Wildlands": 0,
    "Qasridan": 1,
    "Meridian": 2,
    "Valthir": 3,
...
def fix_index_column(state_folder):
    """Fix Index column in burgs.md by restoring numeric indices from burgs.json."""
    ...
    for line in lines:
        ...
        if '|' in line and line.strip().startswith('|') and line.count('|') >= 6:
            ...
            if len(parts) >= 7:
                ...
                if current_index in culture_name_to_index:
                    # Get correct index from JSON
                    correct_index = name_to_index.get(burg_name, current_index)
...
print("Fixing Index column in burgs.md files...")
for state_folder in states_dir.iterdir():
    if state_folder.is_dir() and (state_folder / "burgs.md").exists() and (state_folder / "burgs.json").exists():
        if fix_index_column(state_folder):
