# Hegemon States Index

**Total States:** 21  
**Source:** Azgaar Fantasy Map Generator Export  
**Era:** Hacutonce Era (374 HE)

**Quick Reference:** See [STATES_MASTER_LIST.md](STATES_MASTER_LIST.md) for a comprehensive master list of all states with key statistics and quick reference tables.

## States Directory

This directory contains detailed data for each state in the Hegemon world map. Each state has its own folder with organized data files.

### State List

| Index | Name | Full Name | Type | Form | Capital |
|-------|------|-----------|------|------|---------|
| 0 | Neutrals | Neutrals | Unknown | Unknown | #N/A |
| 1 | Lurland | Regency of Lurland | Generic | Regency | #1 |
| 2 | Trevalia | Regency of Trevalia | Generic | Regency | #2 |
| 3 | Tormaline | Republic of Tormaline | Generic | Republic | #3 |
| 4 | Agrima | Agriman Caliphate | Hunting | Caliphate | #4 |
| 5 | Niljalia | Niljalian Empire | Nomadic | Empire | #5 |
| 6 | Siyafar | Kingdom of Siyafar | Nomadic | Kingdom | #6 |
| 7 | Naiersbia | Naiersbian Theocracy | Generic | Theocracy | #7 |
| 8 | Pontena | Principality of Pontena | Generic | Principality | #8 |
| 9 | Shibara | Emirate of Shibara | Nomadic | Emirate | #9 |
| 10 | Alveimo | Kingdom of Alveimo | Hunting | Kingdom | #10 |
| 11 | Brinach | Principality of Brinach | Generic | Principality | #11 |
| 12 | Kutiograd | Kutiograd Theocracy | Nomadic | Theocracy | #12 |
| 13 | Sealaris | Principality of Sealaris | Generic | Principality | #13 |
| 14 | Kailhen | Protectorate of Kailhen | Generic | Protectorate | #14 |
| 15 | Cournonia | Cournonian Empire | Generic | Empire | #15 |
| 16 | Sigdak | Protectorate of Sigdak | Generic | Protectorate | #16 |
| 17 | Ngathar | Duchy of Ngathar | Generic | Duchy | #17 |
| 18 | Pondara | Dominion of Pondara | Generic | Dominion | #18 |
| 19 | Rutarakia | Protectorate of Rutarakia | Nomadic | Protectorate | #19 |
| 20 | Jarklaus | Kingdom of Jarklaus | Generic | Kingdom | #20 |

## File Structure

Each state folder contains:

- **README.md** - Overview and summary of the state
- **state-data.json** - Complete state data from Azgaar export
- **military.json** - Military units and regiments
- **diplomacy.json** - Diplomatic relations with other states
- **campaigns.json** - Historical campaigns and wars
- **provinces.json** - Province list
- **burgs.json** - Cities and settlements data (complete JSON)
- **burgs.md** - Burgs summary with table of cities
- **coat-of-arms.json** - Coat of arms data (if available)

## Notes

- State indices (i) are used as unique identifiers
- Diplomatic relations use state indices as array positions
- Military data includes unit compositions and locations
- Campaign data includes start/end years and relationships

---

*For Azgaar map data reference, see [../README.md](../README.md)*  
*For Hegemon world documentation, see [../../README.md](../../README.md)*
