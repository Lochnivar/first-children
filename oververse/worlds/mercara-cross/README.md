# Mercara's Cross

**Status:** Proposed World - Concept Development  
**Framework Conformance:** Active - See framework summary in [../mercara-cross.md](../mercara-cross.md)  
**Last Updated:** December 2024

## Overview

Mercara's Cross is the crossroads of Hypatia—the liminal nexus world where portals between all other worlds naturally converge. Mercara created it as a neutral meeting place and invited the triad (Virelle, Nummus, Legara) to build their infrastructure. Virelle built the transit systems including the Grand Anchor; it has organically evolved into a bustling commercial hub.

**Core Identity:** Grand Central Station that incidentally spawned Grand Central Market

## Framework Elements Demonstrated

- **Ownership:** Mercara created the Cross, owns it, coordinates the triad
- **Four FC Structure:** Mercara (owner) + Virelle (transit) + Nummus (banking) + Legara (contracts)
- **Infrastructure Creation:** Grand Anchor (Virelle), portal platforms, Ledger system
- **Soul Transfer Hub:** Central nexus for all inter-world transfers
- **Neutral Ground:** First Children meet for negotiations
- **Commercial Evolution:** Markets emerged organically around transit
- **Specialized Entities:** Kethi as Riftkeepers (Virelle), Elves (Mercara's genetic repo contribution—PR-focused homo), Canis cayminus (Nummus), Kitsune (Legara)

## Quick Reference

### [AI Refresher](AI-REFRESHER.md)
Concise summary for AI context — key facts, notable platforms, file map

## World Structure

### [The Creator: Mercara](creator-mercara.md)
The Cross Owner who created the world, invited the triad, and coordinates communication

### [Core Infrastructure](core-infrastructure.md)
The Grand Anchor, portal platforms, transit halls, Ledger Core

### [Platform List](platform-list.md)
Complete directory of all documented platforms (1,247+ platforms)

### [Geographic Structure](geographic-structure.md)
Central Station District, Station Markets, Support Infrastructure, The Margins

### [Transit Mechanics](transit-mechanics.md)
Standard transfer process, soul transfer costs, safety features

### [Hero Card Trading System](hero-card-trading.md)
How hero trading evolved, legitimate trading process, gray areas, black market

### [The Ledger System](ledger-system.md)
Primary function (transit safety), secondary function (contract enforcement)

### [The Riftkeepers](riftkeepers.md)
Kethi (*Capra Quantus*)—goat-derived transit staff at the Cross; quantum-sensitive; Anchor binding; screaming; the 3 AM chorus

### [Society & Culture](society-culture.md)
Permanent residents, transient population, cultural norms

### [Story Potential](story-potential.md)
Transit stories, market stories, hero card drama, mysteries, liberation operations

### [Themes](themes.md)
The Crossroads Experience, Commerce & Morality, Power & Neutrality

### [Integration with Other Worlds](integration-other-worlds.md)
Connections to Hegemon/Stratara, Delverium, Earth, unknown worlds

---

*For framework-level summary, see [../mercara-cross.md](../mercara-cross.md)*  
*For framework documentation, see [../../FRAMEWORK.md](../../FRAMEWORK.md)*

