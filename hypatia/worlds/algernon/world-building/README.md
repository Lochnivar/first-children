# World-Building Index

Complete index of all world-building elements organized by category.

## The Experiment
- **The Hypothesis:** Phase transition saturation — physical beings → stable waveforms via increasing energetic load (TheWay Strain mechanic). To be developed in `experiment/`, `mechanics/`.

## The Dungeon
- [Central Dungeon](../locations/dungeon/central-dungeon.md) — Delivery mechanism for the experiment
- [Dungeon Levels](../locations/dungeon/levels.md) — Energetic density progression

## TheWay Interface
- [TheWay Interface](theway-interface.md) — How participants see the system: console-debugging aesthetic, wetware (Pineal + Amygdala), player-based mod overlay

## Participant Ecosystem
- **Serious contenders** — Pushing toward invisible finish line
- **Black market souls** — Trafficked for specific quantum profiles
- **Bounty hunters** — Capturing souls for warlords' champions
- **Tourists** — Bucket-list lethal dungeon; occasional outliers
- **Quota fills** — Minimum-spec acquisitions; Charlie's category

## World Context
- [Settlements](../locations/settlements/README.md) — Medieval fantasy world surrounding dungeon (to be developed)
- [Regions](../locations/regions/README.md) — Economics, politics, religion orbiting the dungeon (to be developed)

## Cosmology
- **Keyes** — FC running experiment; present as level-up voice
- **Mira (Kitsune)** — interface-resident waveform fragment; empirical ascension counterpoint
- **Prime Author** — Generates stable existence effortlessly; Keyes's irritation source
- **Algernon (the name)** — Tuesday afternoon decision by bored immortal; inhabitants have elaborate mythologies about what it means

## Templates
- [World-Building Template](template.md) — Template for new entries (to be created if needed)

## Related Documentation
- [Character Index](../characters/README.md) — Charlie, Keyes, Mira, participant types
- [Framework: Algernon](../../algernon.md) — Framework-level summary
